...
class Measurements:
    ...
    def set_list(self, list): #this is is the method that sets the boundaries of the list, and allows it to stay in the bounds of the screen
            self.list = list 
...
def Draw(draw_info, sorting_algorithm_name, acending):
     ...
     #these buttons actully initiate the sorting algorithm
     sorting = draw_info.SMALL_FONT.render("CHOOSE TYPE OF SORT:  I - INSERTION | B - BUBBLE  | G - COUNTING | S - SELECTION  | C - SHELL",1, draw_info.WHITE)
...
def TimeComplexity(sorting_algorithm_name, acending):
     if sorting_algorithm_name == "BUBBLE SORT":
          ...
          return "O(nlogn)"
     elif sorting_algorithm_name == "QUCIK SORT":
          return "O(nlogn)"
...
#this is just a regular ass bubble sort functions
